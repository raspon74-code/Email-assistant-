...
NEW FEATURES:
✅ OAS API Integration (live vessel list with jetty + TBN filters)
✅ OAS Token Helper (oas_token.txt file-based token management)

CARRIED FORWARD FROM v20.0:
...
def get_oas_token():
    ...
    if not os.path.exists(token_file):
        log("⚠️ OAS: oas_token.txt not found — run refresh_token_helper_TOOL.py each morning")
        return None
    try:
        with open(token_file, "r") as f:
            token = f.read().strip()
        if not token:
            log("⚠️ OAS: oas_token.txt is empty — run refresh_token_helper_TOOL.py")
            return None
        return token
...
def fetch_oas_vessels():
    """Fetch vessel list from OAS API, apply jetty + TBN filters, update KNOWN_VESSELS and timeline."""
    ...
    try:
        ...
        response = requests.post(
            OAS_API_URL, json=payload,
            headers={"Content-Type": "application/json", "Origin": "https://nl.oas.shell.com", "User-Agent": "Mozilla/5.0"},
        ...
        if response.status_code in (401, 403):
            log("⚠️ OAS token expired — please run refresh_token_helper_TOOL.py")
            return {}, []
        ...
        for record in data:
            ...
            if jetty_key and jetty_key not in OAS_ALLOWED_JETTIES:
                log(f"   ⏭️ Skipping {name} at jetty {berth} (not in allowed list)")
                continue

            grade = ""
            grades = record.get("GRADES") or []
            if grades and isinstance(grades, list):
                grade = ", ".join([g.get("GRADE_DESC", "") for g in grades if g.get("GRADE_DESC")][:2])
            ...
            oas_vessels.append({
                ...
                "eta": eta, "etd": etd,
                "berth": berth, "jetty": jetty_key,
                "cargo": grade, "grade": grade,
                "agent": agent,
                "surveyor": surveyor if surveyor not in ("NIET VAN TOEPASSING", "NONE", "") else "",
                "status": status_label, "status_desc": status_label,
                "shipment_ref": shipment_ref, "source": "OAS"
            })

        KNOWN_VESSELS.update(new_known)
        log(f"✅ OAS: {len(new_known)} vessels fetched, KNOWN_VESSELS updated.")
        ...
        log(f"📊 Timeline updated: {len(oas_vessels)} OAS vessel entries merged.")
...
def run_summary_agent():
    ...
    log("✅ OAS API Integration | ✅ Live Vessel List")
    log("=" * 60)


    try:
        _, oas_timeline_vessels = fetch_oas_vessels()
        timeline = load_timeline()
...
if __name__ == "__main__":
    try:
        ...
        log("🌐 OAS API Integration Active")
        run_summary_agent()
