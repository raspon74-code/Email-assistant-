"""
Version 21.0 FINAL – Email Assistant - OAS LIVE VESSEL DATA

NEW IN v21:
✅ OAS API integration — live vessel/jetty data from Shell OAS portal
✅ Full cookie bundle authentication (OAS-TOKEN + session cookies)
✅ parse_cookie_string() helper for browser cookie header replay
✅ Graceful fallback to KNOWN_VESSELS when OAS is unavailable
✅ Defensive OAS_SITE default ("PERNIS") with warning log

EXISTING FEATURES (v20):
...
# Files written by refresh_token_helper_TOOL.py
OAS_TOKEN_FILE = "oas_token.txt"
OAS_COOKIES_FILE = "oas_cookies.txt"

# OAS REST API base URL
OAS_API_BASE = "https://oas.shell.com/api"

# Site/terminal name sent as a query parameter.
# If empty or None at runtime it is defaulted to "PERNIS" with a warning.
OAS_SITE = "PERNIS"

# Only include vessels assigned to these jetties
OAS_ALLOWED_JETTIES = {"ST4", "ST17", "ST18", "ST35", "ST35A"}

# Vessel names starting with these prefixes are excluded (TBN / placeholder entries)
OAS_EXCLUDE_NAME_PREFIXES = ("TBN", "TBA", "T.B.N", "T.B.A")
...
# =========================================================
# OAS API — COOKIE HELPER & TOKEN LOADER
# =========================================================

def parse_cookie_string(cookie_str):
    """
    Parse a raw 'key=value; key2=value2' cookie header string into a dict
    suitable for use as the ``cookies`` parameter of a ``requests`` call.

    The leading 'Cookie: ' prefix is stripped automatically if present,
    so the user can paste either the full header line or just the value.
    """
    cookies = {}
    if not cookie_str:
        return cookies
...
def get_oas_token():
    """
    Load the OAS authentication credentials saved by refresh_token_helper_TOOL.py.

    Returns a tuple (token: str | None, cookies: dict).

    Behaviour:
    - If ``oas_token.txt`` is missing → returns (None, {}).
    - If ``oas_cookies.txt`` exists → parses it and returns the full cookie dict
      (which already contains OAS-TOKEN).
    - If ``oas_cookies.txt`` is missing → falls back to {"OAS-TOKEN": token} and
      logs a warning explaining that the full bundle is required.
    """
    token = None
    cookies = {}

    # Load bare token value
    if os.path.exists(OAS_TOKEN_FILE):
        try:
            with open(OAS_TOKEN_FILE, "r", encoding="utf-8") as f:
                token = f.read().strip()
        except Exception as exc:
            log(f"⚠️  Could not read {OAS_TOKEN_FILE}: {exc}")

    if not token:
        log(f"⚠️  OAS token file '{OAS_TOKEN_FILE}' missing or empty — run refresh_token_helper_TOOL.py")
        return None, {}

    # Load full cookie bundle (preferred)
    if os.path.exists(OAS_COOKIES_FILE):
        try:
            with open(OAS_COOKIES_FILE, "r", encoding="utf-8") as f:
                raw = f.read().strip()
            cookies = parse_cookie_string(raw)
            if "OAS-TOKEN" not in cookies:
                # Ensure the token is always present even if the saved string
                # was somehow incomplete
                cookies["OAS-TOKEN"] = token
            log("🍪 OAS full cookie bundle loaded")
        except Exception as exc:
            log(f"⚠️  Could not read {OAS_COOKIES_FILE}: {exc} — falling back to token-only")
            cookies = {"OAS-TOKEN": token}
    else:
        log(
            f"⚠️  '{OAS_COOKIES_FILE}' not found — sending OAS-TOKEN only. "
            "The OAS server may reject the request with HTTP 500. "
            "Run refresh_token_helper_TOOL.py to capture the full cookie bundle."
        )
        cookies = {"OAS-TOKEN": token}
...
# =========================================================
# OAS API — LIVE VESSEL FETCH
# =========================================================

def fetch_oas_vessels():
    """
    Fetch live vessel/jetty data from the OAS REST API.

    Uses the full browser cookie bundle (OAS-TOKEN + session cookies) to
    authenticate.  Falls back to KNOWN_VESSELS if the API is unavailable or
    credentials are missing.

    Returns a list of vessel dicts, or an empty list on failure.
    """
    # Validate / default the site parameter without mutating the module-level variable
    site = OAS_SITE if OAS_SITE else "PERNIS"
    if not OAS_SITE:
        log("⚠️  OAS_SITE is empty — defaulting to 'PERNIS'")

    token, cookies = get_oas_token()
    if not token:
        log("⚠️  No OAS token — skipping live vessel fetch")
        return []

    url = f"{OAS_API_BASE}/vessels"
    params = {"site": site}

    try:
        log(f"🌐 Fetching OAS vessels from {url} (site={site}) …")
        ...
        for v in vessels_raw:
            name = (v.get("vesselName") or v.get("name") or "").strip().upper()
            jetty = (v.get("jetty") or v.get("berth") or "").strip().upper()

            # Filter: only allowed jetties
            if OAS_ALLOWED_JETTIES and jetty not in OAS_ALLOWED_JETTIES:
                continue

            # Filter: exclude TBN / placeholder names
            ...
            results.append({
                "name": name,
                "jetty": jetty,
                "eta": v.get("eta") or v.get("ETA"),
                "etd": v.get("etd") or v.get("ETD"),
                "cargo": v.get("cargo") or v.get("cargoType") or "TBC",
                "agent": v.get("agent") or v.get("agentName") or "",
                "imo": v.get("imo") or v.get("IMO") or "",
                "status_desc": v.get("status") or v.get("statusDescription") or "",
            })

        log(f"✅ OAS returned {len(results)} vessels (after filtering)")
        return results

    except requests.exceptions.HTTPError as exc:
        log(f"❌ fetch_oas_vessels error: {exc}")
        try:
            log(f"OASError: {exc.response.text[:300]}")
        except Exception:
            pass
        return []
    except Exception as exc:
...
def run_summary_agent():
    log("=" * 60)
    log("🚀 Email Assistant v21.0 FINAL — OAS LIVE VESSEL DATA")
    log("✅ Email-to-Checklist | ✅ Anchored Date | ✅ OAS Integration")
    log("=" * 60)
    ...
    try:
        ...
        # OAS live vessel data — merge into timeline if available
        oas_vessels = fetch_oas_vessels()
        if oas_vessels:
            existing_names = {v['name'] for v in timeline.get('vessels', [])}
            added = 0
            for ov in oas_vessels:
                if ov['name'] not in existing_names:
                    timeline.setdefault('vessels', []).append(ov)
                    existing_names.add(ov['name'])
                    added += 1
            if added:
                log(f"✅ OAS: {added} new vessel(s) merged into timeline")
                save_timeline(timeline)
        else:
            log("ℹ️  OAS data unavailable — using KNOWN_VESSELS fallback")
