...
OAS_API_URL     = "https://nl.oas.shell.com/api/shipmentplanner/shipments"
OAS_SITE        = "CVP-D"
OAS_STATUS_FROM = 1
OAS_STATUS_TO   = 9
OAS_DAYS_BACK   = 3
...
def fetch_oas_vessels():
    ...
    try:
        ...
        # Handle both response shapes: plain list OR {"Items": [...]}
        if isinstance(raw, dict):
            data = raw.get("Items") or raw.get("items") or []
            log(f"📦 OAS response is a dict — using Items key ({len(data)} records)")
        ...
        log(f"🔍 OAS first record (field-name check): {data[0]}")

        STATUS_MAP = {
            1: "Nominated", 2: "Confirmed", 3: "Planned", 4: "In Port",
            5: "Trading/Supply", 6: "Scheduling", 7: "Released to operations",
            8: "Sailing", 9: "Arrived", 10: "Completed"
        }

        # Jetties we WANT — ST17, ST18, ST35, ST35A, ST4 under CVP-D
        ALLOWED_JETTIES = {"ST4", "ST17", "ST18", "ST35", "ST35A"}

        # Name prefixes we NEVER want
        ...
        for record in data:
            ...
            # Skip placeholders / TBN entries
            if any(name.startswith(p) for p in EXCLUDE_PREFIXES):
                log(f"   ⏭️ Skip placeholder: {name}")
                continue

            # Berth / jetty
            berth = str(record.get("BERTH") or record.get("BerthCode") or "").strip()

            # Normalize berth code: upper-case, remove dashes and spaces
            jetty_norm = berth.upper().replace("-", "").replace(" ", "")

            # If a berth is assigned and it is NOT in our allow-list → skip
            if berth and jetty_norm not in ALLOWED_JETTIES:
                log(f"   ⏭️ Skip {name} — jetty '{berth}' not in allowed list")
                continue

            # No berth yet → include (vessel is scheduled but not yet berthed)
            if not berth:
                log(f"   ✅ Include {name} — no berth assigned yet")

            # Identifiers (IMO preferred, then ENI)
            ...
            # Dates
            eta = str(record.get("ETA") or record.get("Eta") or record.get("DateFrom") or "").strip()
            etd = str(record.get("ETD") or record.get("Etd") or record.get("DateTo") or "").strip()

            # Grade — try nested GRADES list first, then flat field
            grade = ""
            grades = record.get("GRADES") or []
            if isinstance(grades, list) and grades:
                grade = ", ".join(
                    g.get("GRADE_DESC", "") for g in grades if g.get("GRADE_DESC")
                )[:60]
            if not grade:
                grade = str(record.get("GradeName") or record.get("Grade") or "").strip()

            # Other fields
            agent    = str(record.get("AGENT_ID")     or record.get("AgentName")    or "").strip()
            surveyor = str(record.get("INSPECTOR_ID") or record.get("SurveyorName") or "").strip()
            if surveyor in ("NIET VAN TOEPASSING", "NONE"):
                surveyor = ""
            shipment_ref = str(record.get("SHIPMENT_REF") or record.get("ShipmentRef") or "").strip()
            site         = str(record.get("SITE") or "").strip()

            # Status
            status_code = (
                record.get("NOM_NSTATUS") or
                record.get("SHIPMENT_STATUS") or
                record.get("StatusFrom") or 0
            )
            try:
            ...
            log(f"   ✅ Include {name} @ {berth or 'TBD'}")

        KNOWN_VESSELS = new_known
        log(f"✅ OAS: {len(oas_vessels)} vessels fetched (after filtering).")
        ...
        # Remove stale OAS vessels that are no longer in current OAS results
        oas_names = {v["name"] for v in oas_vessels}
        timeline["vessels"] = [
            v for v in existing.values()
            if v.get("source") != "OAS" or v["name"] in oas_names
        ]

        save_timeline(timeline)
        log(f"📊 Timeline: {updated} OAS vessels merged, {len(timeline['vessels'])} total.")
