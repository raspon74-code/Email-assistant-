...
✅ OAS Vessel Refresh Every Cycle
✅ Status Change Detection Between Cycles
✅ OAS Token Age Monitoring
...
OAS_STATUS_CACHE_FILE = "oas_status_cache.json"
...
def send_summary_to_teams(emails, events, weather, vessels_info, pilot_status, timeline, conflicts, checklist_summary, checklists, delays_detected, status_changes=None):
    try:
        ...
        if status_changes:
            card_body.append({
                "type": "TextBlock",
                "text": f"⚡ OAS Status Changes This Cycle ({len(status_changes)})",
                "size": "Large",
                "weight": "Bolder",
                "color": "Warning",
                "separator": True,
            ...
            for chg in status_changes[:6]:
                card_body.append({
                    "type": "TextBlock",
                    "text": f"{chg['emoji']} **{chg['vessel']}** — {chg['field']}: {chg['old_value']} → {chg['new_value']}",
                    "wrap": True,
                    "spacing": "Small"
                })
...
def check_oas_token():
    """Check OAS token file exists and is non-empty. Logs warnings if not."""
    token_file = "oas_token.txt"
    if not os.path.exists(token_file):
        log("⚠️ OAS TOKEN MISSING: Create oas_token.txt with your OAS-TOKEN cookie value")
        log("   Run refresh_token_helper.py to capture a fresh token from your browser")
        return False
    with open(token_file) as f:
        token = f.read().strip()
    if not token:
        log("⚠️ OAS TOKEN EMPTY: oas_token.txt exists but is empty")
        log("   Run refresh_token_helper.py to capture a fresh token from your browser")
        return False
    # Basic age check: warn if token file is older than 8 hours
    age_hours = (time.time() - os.path.getmtime(token_file)) / 3600
    if age_hours > 8:
        log(f"⚠️ OAS TOKEN AGE: Token file is {age_hours:.1f}h old — may be expired")
        log("   Run refresh_token_helper.py if OAS calls start returning 401/403")
    return True


def fetch_oas_vessels():
    """
    POST to the OAS API, merge results into jetty_timeline.json,
    preserve email-sourced fields, remove stale vessels.
    Returns (KNOWN_VESSELS dict, timeline_vessels list).
    """
    try:
        token_file = "oas_token.txt"
        if not os.path.exists(token_file):
            log("⚠️ fetch_oas_vessels: oas_token.txt not found — skipping OAS fetch")
            timeline = load_timeline()
            return KNOWN_VESSELS, timeline.get("vessels", [])
        with open(token_file) as f:
            oas_token = f.read().strip()
        if not oas_token:
            log("⚠️ fetch_oas_vessels: oas_token.txt is empty — skipping OAS fetch")
            timeline = load_timeline()
            return KNOWN_VESSELS, timeline.get("vessels", [])
        ...
        payload = {
            ...
            "dateFrom": (now - _td(days=OAS_DAYS_BACK)).strftime("%Y-%m-%dT00:00:00"),
            "dateTo": (now + _td(days=OAS_DAYS_AHEAD)).strftime("%Y-%m-%dT23:59:59"),
        }

        log(f"🌐 POSTing to OAS API: {OAS_API_URL}")
        resp = requests.post(
            OAS_API_URL,
            json=payload,
            headers=headers,
        ...
        if resp.status_code in (401, 403):
            log(f"❌ OAS API returned {resp.status_code} — token may be expired. Skipping OAS update.")
            timeline = load_timeline()
            return KNOWN_VESSELS, timeline.get("vessels", [])

        resp.raise_for_status()
        oas_data = resp.json()
        log(f"✅ OAS API responded with {len(oas_data) if isinstance(oas_data, list) else 'non-list'} records")

    except Exception as e:
        log(f"⚠️ OAS API error: {e}")
        timeline = load_timeline()
        return KNOWN_VESSELS, timeline.get("vessels", [])
    ...
    try:
        timeline = load_timeline()
        existing = {v["name"]: v for v in timeline.get("vessels", []) if v.get("name")}

        oas_names_seen = set()
        new_vessels = []

        records = oas_data if isinstance(oas_data, list) else oas_data.get("data", [])

        for rec in records:
            name = str(rec.get("vesselName") or rec.get("vessel_name") or rec.get("name") or "").strip().upper()
            if not name:
                continue
            if any(name.startswith(p) for p in OAS_EXCLUDE_NAME_PREFIXES):
                continue

            jetty = str(rec.get("jettyCode") or rec.get("jetty") or rec.get("berth") or "").strip().upper().replace("-", "").replace(" ", "")
            if jetty and jetty not in OAS_ALLOWED_JETTIES:
                continue

            oas_names_seen.add(name)

            # Build vessel dict — preserve email-sourced fields from existing entry
            prev = existing.get(name, {})
            vessel = {
                "name": name,
                "jetty": jetty or prev.get("jetty", ""),
                "eta": str(rec.get("eta") or rec.get("ETA") or prev.get("eta", "")),
                "etd": str(rec.get("etd") or rec.get("ETD") or prev.get("etd", "")),
                "cargo": str(rec.get("cargoDescription") or rec.get("cargo") or prev.get("cargo", "TBC")),
                "agent": str(rec.get("agentName") or rec.get("agent") or prev.get("agent", "")),
                "imo": str(rec.get("imo") or rec.get("IMO") or prev.get("imo", "")),
                "status": str(rec.get("status") or rec.get("nstatus") or prev.get("status", "")),
                "status_desc": str(rec.get("statusDescription") or rec.get("status_desc") or prev.get("status_desc", "")),
                "surveyor": str(rec.get("surveyor") or prev.get("surveyor", "")),
                # Preserve email-sourced fields
                "anchored_date": prev.get("anchored_date", ""),
                "confidence": prev.get("confidence", ""),
                "email_notes": prev.get("email_notes", ""),
                "source": "OAS",
            }
            new_vessels.append(vessel)

        # Add back any email-only vessels not in OAS (e.g. anchored vessels)
        for name, v in existing.items():
            if name not in oas_names_seen and v.get("source") == "EMAIL":
                new_vessels.append(v)

        timeline["vessels"] = new_vessels
        save_timeline(timeline)
        log(f"✅ OAS merge complete: {len(new_vessels)} vessels in timeline")
        return KNOWN_VESSELS, new_vessels

    except Exception as e:
        log(f"⚠️ OAS merge error: {e}")
        log_exception(e)
        timeline = load_timeline()
        return KNOWN_VESSELS, timeline.get("vessels", [])
...
# =========================================================
# OAS STATUS CHANGE TRACKING
# =========================================================

def load_oas_status_cache():
    """Load previous vessel statuses for change detection."""
    if not os.path.exists(OAS_STATUS_CACHE_FILE):
        return {}
    try:
        with open(OAS_STATUS_CACHE_FILE, "r") as f:
            return json.load(f)
    except:
        return {}


def save_oas_status_cache(vessels):
    """Save current vessel statuses for next-cycle comparison."""
    try:
        cache = {}
        for v in vessels:
            name = v.get("name")
        ...
        with open(OAS_STATUS_CACHE_FILE, "w") as f:
            json.dump(cache, f, indent=2)
    except Exception as e:
        log(f"⚠️ Could not save OAS status cache: {e}")


def detect_oas_status_changes(new_vessels, previous_cache):
    """
    Compare new OAS vessel list against previous cache.
    Returns list of change dicts: {vessel, field, old_value, new_value, emoji}
    """
    changes = []
    try:
        new_by_name = {v["name"]: v for v in new_vessels if v.get("name")}

        # Detect cancellations (in cache but not in new list)
        for name in previous_cache:
            if name not in new_by_name:
                changes.append({
                    "vessel": name,
                    "field": "status",
                    "old_value": previous_cache[name].get("status_desc", "?"),
                    "new_value": "CANCELLED / REMOVED",
                    "emoji": "🚫"
                })

        # Detect additions and changes
        for name, v in new_by_name.items():
            prev = previous_cache.get(name)
            ...
            if prev is None:
                changes.append({
                    ...
                    "field": "status",
                    "old_value": "NEW",
                    "new_value": v.get("status_desc", "?"),
                    "emoji": "🆕"
                })
            else:
                # Status change
                if v.get("status_desc", "") != prev.get("status_desc", ""):
                    changes.append({
                        "vessel": name,
                        "field": "status",
                        "old_value": prev.get("status_desc", "?"),
                        "new_value": v.get("status_desc", "?"),
                        "emoji": "⚡"
                    })
                # ETA change
                if v.get("eta", "") != prev.get("eta", "") and prev.get("eta", "") != "":
                    try:
                        old_eta = datetime.fromisoformat(prev["eta"]).strftime("%b %d %H:%M")
                        new_eta = datetime.fromisoformat(v["eta"]).strftime("%b %d %H:%M")
                        changes.append({
                            "vessel": name,
                            "field": "ETA",
                            "old_value": old_eta,
                        ...
                        pass
                # Jetty change
                if v.get("jetty", "") != prev.get("jetty", "") and prev.get("jetty", "") != "":
                    changes.append({
                        "vessel": name,
                        "field": "jetty",
                        "old_value": prev.get("jetty", "?"),
                        "new_value": v.get("jetty", "?"),
                        "emoji": "🔀"
                    })
    except Exception as e:
        log(f"⚠️ Status change detection error: {e}")
    return changes
...
def run_summary_agent():
    log("=" * 60)
    log("🚀 Email Assistant v21.0 - INTELLIGENT PRODUCTION VERSION")
    log("✅ OAS Refresh Every Cycle | ✅ Status Change Detection")
    ...
    try:
        ...
        # ── OAS: refresh vessel list every cycle ──────────────────
        previous_cache = load_oas_status_cache()
        log("🌐 Refreshing OAS vessel list...")
        _, oas_vessels = fetch_oas_vessels()

        # Detect status changes vs previous cycle
        status_changes = detect_oas_status_changes(oas_vessels, previous_cache)
        if status_changes:
            log(f"⚡ {len(status_changes)} vessel status change(s) detected:")
            for chg in status_changes:
                log(f"   {chg['emoji']} {chg['vessel']}: {chg['field']} {chg['old_value']} → {chg['new_value']}")
        else:
            log("   No vessel status changes since last cycle")

        # Save current statuses for next cycle comparison
        save_oas_status_cache(oas_vessels)
        # ─────────────────────────────────────────────────────────

        timeline = load_timeline()

        # Apply allow-list filter to any cached vessels not yet pruned by fetch_oas_vessels
        ALLOWED_JETTIES  = {"ST4", "ST17", "ST18", "ST35", "ST35A"}
        EXCLUDE_PREFIXES = ("TBN", "UNASSIGNED", "TBA", "T.B.N", "T.B.A", "TOO MUCH")

        def _keep_vessel(v):
            name  = str(v.get("name", "")).strip().upper()
            jetty = str(v.get("jetty", "") or v.get("berth", "")).strip().upper().replace("-", "").replace(" ", "")
            if not name or any(name.startswith(p) for p in EXCLUDE_PREFIXES):
                return False
            if jetty and jetty not in ALLOWED_JETTIES:
                return False
            return True

        before = len(timeline.get("vessels", []))
        timeline["vessels"] = [v for v in timeline.get("vessels", []) if _keep_vessel(v)]
        after = len(timeline["vessels"])
        log(f"📊 Timeline loaded: {before} vessels → {after} after filtering")
        ...
        send_summary_to_teams(
            emails, events, weather, vessels_info, pilot_status,
            timeline, conflicts, checklist_summary, checklists,
            delays_detected, status_changes
        )
        ...
        if status_changes:
            log(f"⚡ {len(status_changes)} status changes flagged")
        log("=" * 60)
...
if __name__ == "__main__":
    try:
        ...
        log("📊 OAS Vessel Refresh Every Cycle")
        log("⚡ Status Change Detection Active")
